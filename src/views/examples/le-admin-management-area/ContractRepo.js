...
var logData1 = {}
function ContractRepo(props) {
  const { dataItem } = props;
  const [key, setKey] = useState('home');
  const [activeTab, setactiveTab] = useState('2')
  const location = useLocation();
  const [openContractRepoModal, setOpenContractRepo] = useState(false);
  const [deleteModal, setDeletToggle] = useState(false);
  const [dataContractRepo, setDataContractRepo] = useState([]);
  const [dataContractRepoUpdate, setDataContractRepoUpdate] = useState([]);
  ...
  useEffect(() => {
    getLoggedData()
    if (activeTab == '2') {
      getDataFiles("CorporateRepository")
    }
  ...
  const toggleUpdateContractRepo = (item) => {
    if (item) {
      setOpenContractRepo(!openContractRepoModal);
      setDataContractRepoUpdate(item)
    }
    else {
  ...
  const ContractRepoCallback = (feeAgrementData) => {
    setIsLoader(true)
    try {
      ...
      if (Object.keys(dataContractRepoUpdate).length !== 0) {
        formData.append('id', dataContractRepoUpdate.id);
      }
      ...
      UploadContractsApi(formData, async (res) => {
        if (res.sucess) {
          ...
          setOpenContractRepo(false)
          await getDataFiles("CorporateRepository")
          setIsLoader(false)
          successAlert(res.sucess.response.messages[0].message)
          setDataContractRepoUpdate([])
  ...
  return (
    <>
      ...
      <div >
              ...
              <div className="col">
                <Card className="shadow">
                  <CardHeader className="border-0">
                    <Row>
                      <h3 className="mb-0">Contract Repository</h3>
                      <Button style={{ position: "absolute", right: 20, top: -12, }} onClick={() => { setOpenContractRepo(!openContractRepoModal) }} className="my-4 p-btm" color="primary" type="button">
                        Add New
                      </Button>
                    </Row>
                                  ...
                                  <td className="text-right">
                                    <UncontrolledDropdown>
                                      ...
                                      <DropdownMenu className="dropdown-menu-arrow" right>
                                        <DropdownItem
                                          onClick={() => { toggleUpdateContractRepo(item) }}>
                                          Update
                                        </DropdownItem>
                                        <DropdownItem
        ...
        <UploadModal modal={openContractRepoModal} data={dataContractRepoUpdate} title='Corporate Repository' parentCallback={ContractRepoCallback} />
